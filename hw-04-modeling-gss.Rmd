...
**Answer:** Using the `count()` function the new levels are agree and disagree. Agree has 1159 answers and disagree has 1708.
...
gss16 <- gss16 %>%
  mutate(polviews = case_when(
    str_detect(polviews, "(?i)liberal") ~ "Liberal",
...
**Answer:** Using the `count()` function the new levels are conservative, moderate, liberal, and NA. Conservative has 928 answers, moderate has 1032, liberal has 796, and NA has 111.
...
The Average Accuracy of the 5-fold cross validation was 0.595.

The Average Area under the **ROC** Curve was 0.534.
...
The Average Area under the **ROC** Curve was 0.529.
...
**Answer:**
The metrics provided above show that the **ROC** curve for Model 1 is 0.534 while the **ROC** curve for Model 2 is 0.529. Based on this, Model 1 is the preferable model for training.
...
I am repeatedly getting this message as I try to answer Question 11...

> Error in `roc_curve()`:
! Can't select columns that don't exist.
✖ Columns `agree`, `Not agree`, `Not agree`, `Not agree`, `agree`, etc. don't exist.
Run `rlang::last_trace()` to see where the error occurred.

After spending 2 and a half hours on this and looking for any solution on the internet. I believe it is in my best interest to skip this question. Thank you for your understanding!
...
count_responses <- count(harass_subset, harass5)

count_responses
```

**Answer:**
This subset shows that 1136 of the responses were no while 237 were yes.
...
**Answer:** Bootstrapping can help determine the estimated proportion of Americans who have been harassed at work by showing averages that can be applied to the greater American population.
...
harassment_data <- filter(gss16, !is.na(harass5) & harass5 %in% c("Yes", "No"))

num_bootstraps <- 1000

calculate_proportion <- function(data) {
  return(sum(data == "Yes") / length(data))
}

set.seed(1116)
bootstrap_proportions <- replicate(num_bootstraps, {
  sample_data <- sample(harassment_data$harass5, replace = TRUE)
  calculate_proportion(sample_data)
})

confidence_interval <- quantile(bootstrap_proportions, c(0.025, 0.975))

confidence_interval
```

**Answer:**
After bootstrapping, the data shows that with 97.5% confidence, 19.37% of Americans are harassed at work. However, with 2.5% confidence, this percentage could be as low as 15.44%.
...
**Answer:**
With a 90% confidence interval it could be expected that it would be narrower than the prior interval. This is because, a higher confidence level requires a wider interval and risks more uncertainty. The 90% interval looks at a smaller and narrower percentage of people than the prior.
